...
export default function RenderTeam({ teamDetail }: { teamDetail: any }) {
  const certifications = teamDetail?.teamsertifikat;
  const teamprojects = teamDetail?.teamproject;
  const skills = teamDetail?.teamskill;
  ...
  return (
    <section className="overflow-hidden min-h-screen">
      <header
        ...
        style={{
          background: "linear-gradient(90deg, #4F4F4F 0%, #959595 100%)",
        }}
      >
      ...
      <div className="max-w-6xl px-5 mx-auto -mt-[120px] z-10 relative ">
        <div className="rounded-full">
          <Image
            src={teamDetail?.image || "/assets/images/antoni.svg"}
            alt="blog header"
            loading="lazy"
            width={600}
            height={600}
      ...
      <div className="max-w-6xl px-5 mx-auto bg-[#F4F4F4] pb-10">
        <div className="bg-gradient-to-r from-gray-700 to-gray-300 pt-10 pb-5 flex justify-between gap-3 md:gap-10">
          <div className="w-9/12 flex flex-col gap-3">
            <h1 className="text-webJudul font-bold">{teamDetail?.name}</h1>
            <p className="text-webSubjudul font-semibold">
              {teamDetail?.title}
            </p>
            <p className="text-webDesk items-stretch flex">
              {removeHTMLTags(teamDetail?.description)}
            </p>
            <div className="font-bold text-webDesk flex flex-col gap-1">
              <p>{teamDetail?.institute}</p>
              <p>{teamDetail?.major}</p>
            </div>
          </div>

          <div className="w-3/12 flex flex-col text-webDesk gap-2">
            <h1>Join Date : {convertJoinDate(teamDetail?.joinDate)}</h1>
        ...
        <div className="bg-white shadow-lg rounded-lg p-5 mb-8 md:px-10 md:py-8">
          <h2 className="text-gray-900 font-extrabold text-webJudul pb-5">
            Contact Info
          </h2>
          <div className="flex flex-wrap gap-6 mt-4 w-full">
            <div className="w-[45%] flex flex-col gap-5">
              <div className="flex gap-3 items-start">
                <div>
                  <Image
                    src="/assets/icons/teams/linkedin.svg"
                    alt="Logo"
                    width={20}
                    height={20}
                    className="w-5 h-5"
                ...
                <div className="-mt-1">
                  ...
                  <Link
                    href={teamDetail?.linkedin || "/"}
                    target="_blank"
                    className="text-webDesk text-[#480DEC]"
                  >
                    {teamDetail?.linkedin}
                  </Link>
                </div>
              ...
              <div className="flex gap-3 items-start">
                <div>
                  <Image
                    src="/assets/icons/teams/email.svg"
                    alt="Logo"
                    width={20}
                    height={20}
                    className="w-5 h-5"
                ...
                <div className="-mt-1">
                  ...
                  <p className="text-webDesk text-[#480DEC]">
                    {teamDetail?.email}
                  </p>
                </div>
              </div>
            ...
            <div className="w-[45%] flex flex-col gap-5">
              <div className="flex gap-3 items-start">
                <div>
                  <Image
                    src="/assets/icons/teams/addres.svg"
                    alt="Logo"
                    width={20}
                    height={20}
                    className="w-5 h-5"
              ...
              <div>
                <div className="flex gap-3 items-start">
                  <div>
                    <Image
                      src="/assets/icons/teams/bday.svg"
                      alt="Logo"
                      width={20}
                      height={20}
                      className="w-5 h-5"
                  ...
                  <div className="-mt-1">
                    ...
                    <p className="text-webDesk">{teamDetail?.birthdayDate}</p>
                  </div>
                </div>
              </div>
            </div>
              ...
              certifications.map((certification: any, index: number) => (
                <div key={index} className="border rounded-md shadow-sm">
                  ...
                  <div className="px-10 py-5">
                    <h1 className="text-webSubjudul font-semibold uppercase">
                      {certification?.title}
                    </h1>
                    <h2 className="text-webSubjudul">
  ...
  return (
    <div className="border rounded-md shadow-sm p-4 flex items-start gap-5">
      ...
      <div className="w-10/12 flex flex-col gap-1">
        <h1 className="text-webJudul font-semibold">{certification?.title}</h1>
        <h2 className="text-webSubjudul font-semibold">
          {certification?.publisher}
        </h2>
        <p className="text-webDesk">
          Issued {convertDate(certification?.startDate)}. Expires{" "}
          {convertDate(certification?.finishDate)}
        </p>
        <p className="text-webDesk">
        ...
        <div>
          <Link
            ...
            target="_blank"
            className="inline-flex mt-2 px-4 py-2 border rounded-xl text-blue-500 border-gray hover:font-semibold items-center gap-3"
          >
  ...
  return (
    <div className="border rounded-md shadow-sm p-4 flex items-start gap-5">
      <div className="w-full flex flex-col gap-2">
        <h1 className="text-webJudul font-semibold">
          {teamproject?.projectName}
        </h1>
        <p className="text-webDesk">
          {convertDate(teamproject?.startDate)} -{" "}
          {convertDate(teamproject?.finishDate)}
        </p>
        <p className="text-webDesk">{teamproject?.description}</p>
        <div>
          <Link
            href={teamproject?.url || "/"}
            className="mt-2 px-4 py-2 border rounded-[20px] text-blue-500 border-black hover:font-semibold inline-flex items-center gap-3"
          >
